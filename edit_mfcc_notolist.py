...
class FOOTSTEP_DATASET():
    ...
    def __init__(self, audio_path, step_cut=1, augmentation=True):
        ...
        # ラベル設定読み込み
        with open("category_7.csv", mode="r") as f:
        ...
        for i, audio_file in enumerate(tqdm(audio_path)):
            ...
            if name not in ["egawa", "harada", "hoshi", "kasai", "kushida", "sakuma", "taguti"]:
                continue
            if step_cut > 0:
                # 足音切り取り
        ...
        for i, d in enumerate(tqdm(self.result_data)):
            ...
            self.result_data[i] = feature_melsp_db
...
def main():
    ...
    for const in calc_list:
        ...
        # データセット作成
        dataset = FOOTSTEP_DATASET(audio_path=audio_path, augmentation=True, step_cut=2)
        ...
        dataset.save_npz(
            ...
            shuffle=True,
            is_upload_s3=False,
            test_npz_save_path="./npz_メルスペクトグラム",
            train_npz_save_path="./npz_メルスペクトグラム",
